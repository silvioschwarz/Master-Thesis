...
DMBI04_2_Catalogue = function(filename_in, eqlist, filename_out){
                ...
                for(i in 1:nrow(data)){
                        Date[i] = format(as.Date(paste(data$Gi[i],data$Me[i], 
                                                       data$An[i]),format("%d %m %Y")), 
                                         format="%d/%m/%Y")
                        
                        R[i] = diag(rdist.earth(matrix(c(data$LonOr[i], data$LatOr[i]), ncol = 2), 
                                                matrix(c(data$LonEp[i], data$LatEp[i]), ncol = 2), 
                                                miles = FALSE, R = 6371))
                } 
                
                daten = data.frame(ID = 1, Datum = Date, 
                                   Ix = data$Ix, I0 = data$Io, Mw = data$Mw, Mw_err = data$Daw,
                                   Is = data$IsOr/10,  R = R, 
                                   LatEp = data$LatEp, LonEp = data$LonEp, 
                                   LatOr = data$LatOr, LonOr = data$LonOr )
